...
- **Terminal states with a paper trail.** Every session ends in exactly one of done-and-verified / blocked / deviation / crashed-predecessor — written to the journal, stamped with the executor's model ID, committed to git, and surfaced as a one-line `NEXT:` the user can't miss. Silence never resolves to done.
